...
impl Annotation for TextBox {
    fn to_buffer(&self) -> Vec<AnnotationElement> {
        ...
        let (left, right, top, bottom) = self.ltrb;
        ...
        let text = string_to_lines(&self.text, self.font_size, width - 2.*self.padding);

        // Get starting y coordinate based on text alignment
        let mut y: f32 = match self.vertical_alignment {
            VerticalAlignment::Top => top + self.padding,
            VerticalAlignment::Middle => {
                let total_line_height: f32 = self.line_spacing * self.font_size * text.len() as f32;
                0.5 * (bottom - top - total_line_height)
            },
            VerticalAlignment::Bottom => {
                let total_line_height: f32 = self.line_spacing * self.font_size * text.len() as f32;
                bottom - self.padding - total_line_height
            }
        };
